...
int SensorBase::find_type_by_name(const char *name, const char *type) {
    ...
    DIR *dp;
    char thisname[30];
    char filename[IIO_DIR_MAX_LENGTH];
    ...
    while (ent = readdir(dp)) {
        if (strcmp(ent->d_name, ".") != 0 &&
            ...
            snprintf(filename,IIO_DIR_MAX_LENGTH, "%s%s%d/name", iio_dir, type, number);
            nameFile = fopen(filename, "r");
            if (!nameFile) {
                SH_ERR("fopen %s failed!", filename);
                continue;
            }
            ret1=fscanf(nameFile, "%s", thisname);
            if (ret1<0){
                SH_ERR("fscanf %s failed!", filename);
                fclose(nameFile);
                continue;
            }
